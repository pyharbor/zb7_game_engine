...
class GreatBarrierReef(RelicSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=197,
                         sub_type_as_text="GreatBarrierReef", 
                         **kwargs)
    ...
    def start_of_battle(self,
        ...
        for x in info.friendly_recruits:
            s = x.get_status(sub_type_as_text="CoralProtection")
            if s is None:
                continue
            else:
                status = Statuses.CoralProtection(counter=s.counter)
                x.add_status(
