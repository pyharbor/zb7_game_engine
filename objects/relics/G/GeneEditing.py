...
class GeneEditing(RelicSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=213,
                         sub_type_as_text="GeneEditing", 
                         **kwargs)
        ...
        if "type" not in self.custom_data:
            self.custom_data["type"] = None
            self._type = None
        else:
            self._type = self.custom_data["type"]
    ...
    def shop_set_object_data(self, shop_state: "ShopStateSerializer", state_set: "StateSet", stack_item: "StackItem",
        ...
        if user_input.set_shop_object_data_opcode == GameConstants.Opcodes.ObjectData.shop_set_custom:
            ...
            if user_input.player_decision_choice not in GameConstants.ScientificNames.__dict__:
                raise Exception("Invalid species choice")
            ...
            self.custom_data["type"] = user_input.player_decision_choice
            self.target = team_match.shop_id
            self._type = self.custom_data["type"]
            obj: RecruitSerializer = shop_state.get_object_from_shop_id(shop_id=self.target)
            obj.added_types.append(user_input.player_decision_choice)
