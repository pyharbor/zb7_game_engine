...
class PerfectCopy(RelicSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=171,
                         sub_type_as_text="PerfectCopy", 
                         **kwargs)
            ...
            if team_match is None:
                ...
                for x in shop_state.friendly_recruits:
                    if x.type == "EmptySlot":
                        ...
                        _copy = team_match.copy()
                        _copy.shop_id = shop_state
                        shop_state.add_recruit(recruit=_copy, index=x.team_index)
                        return
                raise Exception("There is not room on your team to create a copy, sell something first")
        else:
