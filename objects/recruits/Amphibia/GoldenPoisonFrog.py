...
class GoldenPoisonFrog(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=50,
                         sub_type_as_text="GoldenPoisonFrog", 
                         **kwargs)
    ...
    def battle_attack_with_melee(self, battle_state: "BattleStateSerializer",
        ...
        if self.team_index == 0:
            ...
            if self.experience < GameConstants.Levels.level_4:
                status = Statuses.Batrachotoxin(counter=1)
            else:
                status = Statuses.Batrachotoxin(counter=2)
