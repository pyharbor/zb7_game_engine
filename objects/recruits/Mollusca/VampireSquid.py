...
class VampireSquid(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=100,
                         sub_type_as_text="VampireSquid", 
                         **kwargs)
    ...
    def battle_attack_with_melee(self, battle_state: "BattleStateSerializer",
        ...
        if self.team_index == 0 and self.melee_attack > 0:
            ...
            original_health = front_enemy.health
            front_enemy.battle_receive_damage(damage=damage,
                                              ...
                                              animation_event_sequence=animation_event_sequence,
                                              origin="melee")
            health_lost = original_health - front_enemy.health
            mollusk_recruits = [x for x in info.friendly_recruits if GameConstants.ScientificNames.Mollusca in x.binomial_nomenclature]
            g = Group()
            for x in mollusk_recruits:
                x.battle_heal(
                    healer=self,
                    heal=int(health_lost),
                    state_set=state_set,
                    battle_state=battle_state,
                    animation_event_sequence=animation_event_sequence,
                    group=g
    ...
    def battle_attack_with_range(self, battle_state: "BattleStateSerializer",
        ...
        if self.team_index != 0 and self.ranged_attack > 0:
            ...
            info = battle_state.get_battle_info(battle_id=self.battle_id)
            original_health = random_enemy.health
            random_enemy.battle_receive_damage(damage=damage,
                                               ...
                                               animation_event_sequence=animation_event_sequence,
                                               origin="ranged")
            health_lost = original_health - random_enemy.health
            mollusk_recruits = [x for x in info.friendly_recruits if
                                GameConstants.ScientificNames.Mollusca in x.binomial_nomenclature or GameConstants.Habitats.DeepSea in x.binomial_nomenclature]
            g = Group()
            for x in mollusk_recruits:
                x.battle_heal(
                    healer=self,
