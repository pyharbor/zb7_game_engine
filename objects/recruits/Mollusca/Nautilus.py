...
class Nautilus(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=49,
                         sub_type_as_text="Nautilus", 
                         **kwargs)
    ...
    def shop_level_up(self, shop_state: "ShopStateSerializer", state_set: "StateSet", stack_item: "StackItem",
        ...
        index = min(logical_level + 1, len(fib) - 1)
        armor_buff = max(logical_level - 3, 0)
        if index < 0:
            index = 0
        if index > len(fib):
            return

        buff = min(fib[index], 144) + 10
