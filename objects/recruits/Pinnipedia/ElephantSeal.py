...
class ElephantSeal(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=33,
                         sub_type_as_text="ElephantSeal", 
                         **kwargs)
    ...
    def battle_attack_with_melee(self, battle_state: "BattleStateSerializer",
        ...
        if self.team_index == 0 and self.melee_attack > 0:
            ...
            if self.experience < GameConstants.Levels.level_3:
                ...
                amount = 15
            self.battle_receive_damage(damage=amount,
                                       enemy=self,
                                       state_set=state_set,
                                       battle_state=battle_state,
                                       animation_event_sequence=animation_event_sequence,
    ...
    def shop_start_of_turn(self, shop_state: "ShopStateSerializer", state_set: "StateSet", stack_item: "StackItem",
        ...
        seal_count = [x for x in shop_state.friendly_recruits if
                      GameConstants.ScientificNames.Pinnipedia in x.binomial_nomenclature]
        seal_count = len(seal_count)
        self.shop_buff_stats(
            shop_state=shop_state,
            state_set=state_set,
            stack_item=stack_item,
            animation_event_sequence=animation_event_sequence,
            ranged=seal_count * ranged_buff,
            health=seal_count * health_buff,
            max_health=seal_count * health_buff,
            melee=seal_count * melee_buff,
        )
