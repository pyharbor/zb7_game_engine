...
class GreatWhiteShark(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=27,
                         sub_type_as_text="GreatWhiteShark", 
                         **kwargs)
    ...
    def battle_attack_with_melee(self, battle_state: "BattleStateSerializer",
        ...
        if self.team_index == 0 and self.melee_attack > 0:
            ...
            overkill = front_enemy.battle_receive_damage(damage=damage,
                                                         enemy=self,
                                                         state_set=state_set,
                                                         battle_state=battle_state,
                                                         animation_event_sequence=animation_event_sequence,
            ...
            remaining_alive_enemies = [enemy for enemy in info.enemy_recruits if enemy.health > 0]
            while overkill is not None and overkill > 0 and len(remaining_alive_enemies) > 0:
                random_enemy = self.get_completely_random_enemy(battle_state=battle_state)
                if random_enemy is None:
                    return
                damage = overkill

                e = Animations.Overkill(
                    state_id=state_set.add_state(state=battle_state),
                    shop_id=self.shop_id,
                    battle_id=self.battle_id,
                ...
                animation_event_sequence.append(e)
                overkill = random_enemy.battle_receive_damage(damage=overkill,
                                                              enemy=self,
                                                              state_set=state_set,
                                                              battle_state=battle_state,
                                                              animation_event_sequence=animation_event_sequence,
    ...
    def battle_attack_with_range(self, battle_state: "BattleStateSerializer",
        ...
        if self.team_index != 0 and self.ranged_attack > 0:
            ...
            animation_event_sequence.append(e)
            overkill = random_enemy.battle_receive_damage(damage=damage,
                                                          enemy=self,
                                                          state_set=state_set,
                                                          battle_state=battle_state,
                                                          animation_event_sequence=animation_event_sequence,
