...
class QueenAnt(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=54,
                         sub_type_as_text="QueenAnt", 
                         **kwargs)
    ...
    def start_of_battle(self,
        ...
        for x in info.friendly_recruits:
            if GameConstants.ScientificNames.Hymenoptera in x.binomial_nomenclature:
                amount = 1
                status = Statuses.HiveDurability(counter=amount)
                x.add_status(
