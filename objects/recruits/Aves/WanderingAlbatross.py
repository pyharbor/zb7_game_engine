...
class WanderingAlbatross(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=101,
                         sub_type_as_text="WanderingAlbatross", 
                         **kwargs)
    ...
    def start_of_battle(self,
        ...
        self.add_status(Statuses.Airborne(counter=10), battle_state=battle_state, state_set=state_set,
                        animation_event_sequence=animation_event_sequence,
                        enemy=None)
    ...
    def passive_battle_ability(self,
        ...
        for x in self.listeners.get_listeners(hook=Listeners.Hooks.passive_battle_ability):
            ...
            if should_return:
                return
        if self.get_status(sub_type_as_text="Airborne") is not None:
            self.battle_heal(heal=10,
                             battle_state=battle_state,
                             state_set=state_set,
                             healer=self,
                             animation_event_sequence=animation_event_sequence)
