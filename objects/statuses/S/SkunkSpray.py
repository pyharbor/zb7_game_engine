...
class SkunkSpray(StatusSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=219,
                         sub_type_as_text="SkunkSpray", 
                         **kwargs)
    ...
    def lt_battle_attack_with_melee(self, battle_state: "BattleStateSerializer",
        ...
        if self.target.sub_type_as_text == "HoodedSkunk":
            return [damage, False]
        damage = int(damage * (1 - (self.counter * .1)))
        damage = max(0, damage)
        self.counter -= 1
        if self.counter < 0:
            self.target.listeners.remove_listener(hook=Listeners.Hooks.battle_attack_with_range,
                                                  listener=self)
            self.target.listeners.remove_listener(hook=Listeners.Hooks.battle_attack_with_melee,
                                                  listener=self)
    ...
    def lt_battle_attack_with_range(self, battle_state: "BattleStateSerializer",
        ...
        if self.target.sub_type_as_text == "HoodedSkunk":
            return [damage, False]
        damage = int(damage * (1 - self.counter))
        damage = max(0, damage)
        self.counter -= 1
        if self.counter < 0:
            self.target.listeners.remove_listener(hook=Listeners.Hooks.battle_attack_with_range,
