...
class ShellArmor(StatusSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=159,
                         sub_type_as_text="ShellArmor", 
                         **kwargs)
    ...
    def lt_battle_receive_damage(self, damage: int,
        ...
        if damage % 3:
            amount_of_used_counters = damage // 3 + 1
        else:
            amount_of_used_counters = damage // 3

        reduced_damage = int(max(0, int(damage - amount_of_used_counters * 3)))
        ...
        self.counter -= amount_of_used_counters
        if self.counter <= 0:
