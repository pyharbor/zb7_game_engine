...
class Carapace(StatusSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=223,
                         sub_type_as_text="Carapace", 
                         **kwargs)
    ...
    def lt_battle_receive_damage(self, damage: int,
        ...
        if original_damage > 25:
            damage_reduction_stack.append(dict(amount=original_damage - damage,
                                               sub_type_as_int=GameConstants.DamageReductionStack.Carapace))
            self.counter -= 1
            if self.counter <= 0:
                self.target.listeners.remove_listener(hook=Listeners.Hooks.battle_receive_damage,
                                                      listener=self)
