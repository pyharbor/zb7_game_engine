...
class Vengeance(StatusSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=218,
                         sub_type_as_text="Vengeance", 
                         **kwargs)
                         
        # ------ Protect Below from Code Templating ------
        self.prevented_death = False
    ...
    def status_effect(self,
        ...
        if self.counter == 0:
            recruit.listeners.remove_listener(hook=Listeners.Hooks.battle_faint,
                                              listener=self)
    ...
    def lt_battle_faint(self, battle_state: "BattleStateSerializer",
        ...
        if self.counter > 0:
            ...
            self.counter -= 1
            if self.counter == 0:
                self.target.listeners.remove_listener(hook=Listeners.Hooks.battle_faint,
                                                      listener=self)
                self.target.remove_status(status=self)
            e = Animations.Vengeance(state_id=state_set.add_state(state=battle_state),
