...
if __name__ == "__main__":
    ...
    for x in sub_types:
        if x["type"] == "Recruit":
            ...
            matches = re.findall(pattern=single_quote_regex, string=x['ability'])
            if len(matches) > 0:
                for match in matches:
                    key_words[match] += 1
    ...
    for x in sub_types:
        if x["type"] == "Recruit":
            if x["sub_type_as_text"] == "BlueWhale":
                pass
            for i, ability in enumerate(x["ability"]):
                replacement = ability["description"]
                matches = re.findall(pattern=single_quote_regex, string=ability['description'])
                if len(matches) > 0:
                    matches_set = set()
                    for match in matches:
                        if match in matches_set:
                            continue
                        matches_set.add(match)
                        color = key_word_color_map[_json[match]["type"]]
                        replacement = replacement.replace(match, f"[url={match}][color={color}]{match}[/color][/url]")
                x["ability"][i]["description"] = replacement
        elif x["type"] == "Relic":
            for i, ability in enumerate(x["ability"]):
                replacement = ability["description"]
                matches = re.findall(pattern=single_quote_regex, string=ability['description'])
            ...
            if len(matches) > 0:
                ...
                for match in matches:
                    ...
                    color = key_word_color_map[_json[match]["type"]]
                    replacement = replacement.replace(match, f"[url={match}][color={color}]{match}[/color][/url]")
            x["ability"] = replacement

    Disk.Sync.write_file_text(data=json.dumps(sub_types, indent=4), filename="/Users/jacobsanders/Desktop/PythonProjects/cloud_applications8/git/projects/games/zb7_game_engine/zb7_game_engine/immutable_data/configs/sub_types_bbcode.json")
