...
class SandstormDamage(AnimationEventSerializer):
    def __init__(self, state_id: int, shop_id: int, battle_id: int, amount: int):
        super().__init__(animation_type_as_text=GameConstants.Animations.SandstormDamage,
                         state_id=state_id)
    ...
    @classmethod
    def from_bytes(cls, _bytes: bytes) -> "SandstormDamage":
        return BasicIdsAmount.from_bytes(cls, _bytes)


if __name__ == "__main__":
    SandstormDamage(state_id=0)