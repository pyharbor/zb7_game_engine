...
class ShopUpdatedRecruit(AnimationEventSerializer):
    def __init__(self, state_id: int, shop_id: int, battle_id: int):
        super().__init__(animation_type_as_text=GameConstants.Animations.ShopUpdatedRecruit,
                         state_id=state_id)
    ...
    @classmethod
    def from_bytes(cls, _bytes: bytes) -> "ShopUpdatedRecruit":
        return BasicIds.from_bytes(cls, _bytes)


if __name__ == "__main__":
    ShopUpdatedRecruit(state_id=0)