...
class ShopLevelUp(AnimationEventSerializer):
    def __init__(self, state_id: int, shop_id: int, battle_id: int, amount: int):
        super().__init__(animation_type_as_text=GameConstants.Animations.ShopLevelUp,
                         state_id=state_id)
        self.amount = amount
    ...
    @classmethod
    def from_bytes(cls, _bytes: bytes) -> "ShopLevelUp":
        return BasicIdsAmount.from_bytes(cls, _bytes)


if __name__ == "__main__":
    ShopLevelUp(state_id=0)
