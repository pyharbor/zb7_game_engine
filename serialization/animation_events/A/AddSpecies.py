...
class AddSpecies(AnimationEventSerializer):
    def __init__(self, state_id: int, shop_id: int = None, battle_id: int = None, species_as_int: int = None):
        super().__init__(animation_type_as_text=GameConstants.Animations.AddSpecies,
                         state_id=state_id)
        self.shop_id = shop_id
        self.battle_id = battle_id
        self.species_as_int = species_as_int
        self.species_as_text = ImmutableData.ScientificNomenclature.from_int(self.species_as_int)
    ...
    def to_bytes(self) -> bytearray:
        ...
        species_as_int_bytes = self.species_as_int.to_bytes(1, byteorder="big")
    ...
    @classmethod
    def from_bytes(cls, _bytes: bytes) -> "AddSpecies":
        current_index = 0
        ...
        current_index += 1
        species_as_int = int.from_bytes(_bytes[current_index:current_index + 1], byteorder="big")
        current_index += 1
        return cls(state_id=state_id, shop_id=shop_id, battle_id=battle_id, species_as_int=species_as_int)


if __name__ == "__main__":
    AddSpecies(state_id=0)