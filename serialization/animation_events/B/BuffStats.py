...
class BuffStats(AnimationEventSerializer):
    def __init__(self, state_id: int, shop_id: int, battle_id: int, health: int, melee: int, ranged: int, armor: int,
        ...
        super().__init__(animation_type_as_text=GameConstants.Animations.BuffStats,
                         state_id=state_id)
        self.health = health
        self.melee = melee
        self.ranged = ranged
    ...
    @classmethod
    def from_bytes(cls, _bytes: bytes) -> "BuffStats":
        return StatsModification.from_bytes(cls, _bytes)


if __name__ == "__main__":
    BuffStats(state_id=0)
