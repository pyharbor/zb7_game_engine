...
class LightningDamage(AnimationEventSerializer):
    def __init__(self, state_id: int, battle_id: int, shop_id: int, amount: int):
        super().__init__(animation_type_as_text=GameConstants.Animations.LightningDamage,
                         state_id=state_id)
    ...
    @classmethod
    def from_bytes(cls, _bytes: bytes) -> "LightningDamage":
        return BasicIdsAmount.from_bytes(cls, _bytes)


if __name__ == "__main__":
    LightningDamage(state_id=0)