...
class ReceiveStatusDamage(AnimationEventSerializer):
    def __init__(self, state_id: int, battle_id: int, shop_id: int, status_sub_type_as_int: int, amount: int):
        super().__init__(animation_type_as_text=GameConstants.Animations.ReceiveStatusDamage,
                         state_id=state_id)
    ...
    @classmethod
    def from_bytes(cls, _bytes: bytes) -> "ReceiveStatusDamage":
        current_index = 0
...
if __name__ == "__main__":
    ReceiveStatusDamage(state_id=0)