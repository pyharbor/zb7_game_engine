...
class MeleeAttack(AnimationEventSerializer):
    def __init__(self, state_id: int, shop_id: int, battle_id: int, target_shop_id: int, target_battle_id: int, amount: int):
        super().__init__(animation_type_as_text=GameConstants.Animations.MeleeAttack,
                         state_id=state_id)
    ...
    @classmethod
    def from_bytes(cls, _bytes: bytes) -> "MeleeAttack":
        return TargetSubjectAmount.from_bytes(cls, _bytes)


if __name__ == "__main__":
    MeleeAttack(state_id=0)
