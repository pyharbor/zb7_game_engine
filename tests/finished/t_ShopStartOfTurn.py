...
def asdf():
    ...
    r = Recruits.Amphibia.ForestRainFrog()
    shop_snapshot.current_state.add_recruit(recruit=r, index=0)
    r = Recruits.Amphibia.TreeFrog()
    shop_snapshot.current_state.add_recruit(recruit=r, index=1)
    s = ShopUserInput(op=GameConstants.Opcodes.Shop.shop_start_of_turn)
