...
def asdf():
    ...
    friendly_shop_snapshot.current_state.add_recruit(recruit=Recruits.Cetacea.BlueWhale(), index=0)
    ...
    print(json.dumps(battle_snapshot.to_json()).replace("'", '"'))
    print(len(json.dumps(battle_snapshot.to_json()).encode("utf-8")))

    battle_state = battle_snapshot.state_set.get_state_by_state_id(state_id=1, _type="BattleSnapshot")
    print(json.dumps(battle_state.to_json()).replace("'", '"'))
    print(len(json.dumps(battle_state.to_json()).encode("utf-8")))
