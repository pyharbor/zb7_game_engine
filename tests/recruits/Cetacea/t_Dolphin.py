...
def asdf():
    ...
    friendly_shop_snapshot.current_state.add_recruit(recruit=Recruits.Cetacea.Dolphin(), index=0)
    ...
    print(Debug.ShopSnapshotSerializer.print(friendly_shop_snapshot))
    print(Debug.ShopSnapshotSerializer.print(friendly_shop_snapshot))
    print(json.dumps(friendly_shop_snapshot.to_json()).replace("'", '"'))
    print(len(json.dumps(friendly_shop_snapshot.to_json()).encode("utf-8")))

    shop_state = friendly_shop_snapshot.state_set.get_state_by_state_id(state_id=2, _type="ShopSnapshot")
    print(json.dumps(shop_state.to_json()).replace("'", '"'))
    print(len(json.dumps(shop_state.to_json()).encode("utf-8")))
    print(shop_state.friendly_recruits[0])
    shop_state = friendly_shop_snapshot.state_set.get_state_by_state_id(state_id=1, _type="ShopSnapshot")
    print(json.dumps(shop_state.to_json()).replace("'", '"'))
    print(len(json.dumps(shop_state.to_json()).encode("utf-8")))
    print(shop_state.friendly_recruits[0])
