...
def asdf():
    ...
    friendly_shop_snapshot.current_state.add_recruit(recruit=Recruits.Actinopterygii.Barracuda(), index=1)
    friendly_shop_snapshot.current_state.add_recruit(recruit=Recruits.Actinopterygii.AtlanticHerring(), index=0)

    enemy_shop_snapshot.current_state.add_recruit(recruit=Recruits.Arachnida.AsianForestScorpion(), index=0)

    print(friendly_shop_snapshot.friendly_recruits[1].experience)
    ...
    original_shop_state = battle_snapshot.original_shop_state

    print(battle_snapshot.to_json())
    print(len(json.dumps(battle_snapshot.to_json()).encode("utf-8")))
    print(original_shop_state.friendly_recruits[1].experience)
