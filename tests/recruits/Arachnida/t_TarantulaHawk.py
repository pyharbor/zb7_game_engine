...
def asdf():
    ...
    friendly_shop_snapshot.current_state.add_recruit(recruit=Recruits.Arachnida.TarantulaHawk(), index=0)
    a = Recruits.Arachnida.AsianForestScorpion()
    a.shop_id = friendly_shop_snapshot.current_state.get_next_shop_id()
    friendly_shop_snapshot.current_state.shop[0] = a
    ...
    print(friendly_shop_snapshot.to_json())
    print(len(json.dumps(friendly_shop_snapshot.to_json()).encode("utf-8")))
    print(friendly_shop_snapshot.current_state.friendly_recruits[1].added_types)
