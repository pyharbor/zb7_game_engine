...
class Burrowed(StatusSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=158,
                         sub_type_as_text="Burrowed", 
                         **kwargs)
    ...
    def lt_battle_receive_damage(self, damage: int,
        ...
        if origin == "ranged":
            overkill = max(0, damage - 20 * self.counter)
            reduced_damage = damage - overkill
            if damage_reduction_stack is not None:
                damage_reduction_stack.append(
                    dict(amount=reduced_damage, sub_type_as_int=GameConstants.DamageReductionStack.Burrowed))
