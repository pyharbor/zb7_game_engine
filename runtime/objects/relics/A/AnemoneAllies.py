...
class AnemoneAllies(RelicSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=198,
                         sub_type_as_text="AnemoneAllies", 
                         **kwargs)
    ...
    def start_of_battle(self,
        ...
        for x in info.friendly_recruits:
            matched = False
            for habitat in GameConstants.Habitats.Oceanic:
                if habitat in x.habitats:
                    matched = True
                    break
            if not matched:
                continue

            status = Statuses.AnemoneAttachment(counter=1)
            x.add_status(
