...
class GiantPanda(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=133,
                         sub_type_as_text="GiantPanda", 
                         **kwargs)
                         
        # ------ Protect Below from Code Templating ------
        if "bamboo_counter" not in self.custom_data:
            self.custom_data["bamboo_counter"] = 5
        self.honey_counter = self.custom_data["bamboo_counter"]
    ...
    def shop_start_of_turn(self, shop_state: "ShopStateSerializer", state_set: "StateSet", stack_item: "StackItem",
        ...
        melee_buff = int(.35 * self.custom_data["bamboo_counter"])
        ranged_buff = int(.35 * self.custom_data["bamboo_counter"])
        health_buff = int(.75 * self.custom_data["bamboo_counter"])
    ...
    def bytes_to_custom_data(cls, _bytes: bytes, current_index: int) -> dict:
        return Uint16Counter.bytes_to_custom_data(_bytes=_bytes, current_index=current_index, key="bamboo_counter")

    def custom_data_to_bytes(self) -> bytes:
        return Uint16Counter.custom_data_to_bytes(self, key="bamboo_counter")
    ...
    def battle_attack_with_range(self, battle_state: "BattleStateSerializer",
        ...
        if self.team_index != 0:
            ...
            if logical_level < 3:
                bamboo_probability = .9
            elif logical_level < 4:
                bamboo_probability = .3
            else:
                bamboo_probability = .1
            random_num = battle_state.random.random()
            if random_num < bamboo_probability:
                self.generic_ability_notification(
                    state=battle_state,
                    description="Bamboo!",
                    state_set=state_set,
                    animation_event_sequence=animation_event_sequence)
                self.custom_data["bamboo_counter"] += self.get_logical_level() + 1
                if original_shop_state is not None:
                    obj = original_shop_state.get_object_from_shop_id(shop_id=self.shop_id)
                    obj.custom_data["bamboo_counter"] = self.custom_data["bamboo_counter"]
                return None
