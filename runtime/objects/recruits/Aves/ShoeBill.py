...
class ShoeBill(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=238,
                         sub_type_as_text="ShoeBill", 
                         **kwargs)
    ...
    def shop_end_of_turn(self, shop_state: "ShopStateSerializer", state_set: "StateSet", stack_item: "StackItem",
        ...
        aves = [x for x in shop_state.friendly_recruits
        ...
        for x in shop_state.friendly_recruits:
            for h in GameConstants.Habitats.Swampy:
                if h in x.binomial_nomenclature:
                    swamp_recruits.append(x)
                    break
        swamp_boost = len(swamp_recruits)
        aves_boost = len(aves)
