...
class GiantClam(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=99,
                         sub_type_as_text="GiantClam", 
                         **kwargs)
                         
        # ------ Protect Below from Code Templating ------
        self.has_blocked_first_attack = False
        ...
        for x in shop_state.friendly_recruits:
            ...
            if GameConstants.ScientificNames.Mollusca in x.binomial_nomenclature:
                if self.experience < GameConstants.Levels.level_4:
                    ...
                    health_buff += 4
                    ranged_buff += 4
            if GameConstants.ScientificNames.Bivalvia in x.binomial_nomenclature or GameConstants.ScientificNames.Gastropoda in x.binomial_nomenclature:
                if self.experience < GameConstants.Levels.level_4:
                    health_buff += 1
                    ranged_buff += 1
                else:
            ...
            if GameConstants.Habitats.CoralReef in x.binomial_nomenclature:
                armor_buff += 1
                melee_buff += 1
            if ranged_buff > 0 or melee_buff > 0 or health_buff > 0 or armor_buff > 0:
                x.shop_buff_stats(
    ...
    def battle_receive_damage(self, damage: int,
        ...
        if not self.has_blocked_first_attack:
            self.has_blocked_first_attack = True
            if damage_reduction_stack is None:
                damage_reduction_stack = [
                    ...
                    dict(amount=damage, sub_type_as_int=GameConstants.DamageReductionStack.ClamShell)
                ]
            return super().battle_receive_damage(0, battle_state, enemy, state_set, animation_event_sequence,
