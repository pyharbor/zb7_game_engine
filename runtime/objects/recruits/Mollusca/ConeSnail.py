...
class ConeSnail(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=95,
                         sub_type_as_text="ConeSnail", 
                         **kwargs)
    ...
    def passive_battle_ability(self,
        ...
        choice: RecruitSerializer = battle_state.random.choice(info.enemy_recruits)
        choice.add_status(
            battle_state=battle_state,
            state_set=state_set,
            enemy=None,
            animation_event_sequence=animation_event_sequence,
            status=Statuses.NeurotoxicPeptides(counter=20),
        )
    ...
    def shop_start_of_turn(self, shop_state: "ShopStateSerializer", state_set: "StateSet", stack_item: "StackItem",
        ...
        number_of_coastline = len([x for x in shop_state.friendly_recruits if
                                  GameConstants.Habitats.Coastline in x.binomial_nomenclature])
        armor_amount = number_of_coastline * 2
        health_amount = 0 if self.get_logical_level() < 3 else number_of_coastline
        self.generic_ability_notification(
            state_set=state_set,
            animation_event_sequence=animation_event_sequence,
            state=shop_state
        )
        self.shop_buff_stats(armor=armor_amount,
