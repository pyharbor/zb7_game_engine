...
class AmazonRiverDolphin(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=234,
                         sub_type_as_text="AmazonRiverDolphin", 
                         **kwargs)
        ...
        for x in shop_state.friendly_recruits:
            ...
            for h in GameConstants.Habitats.Freshwater:
                if h in x.binomial_nomenclature:
                    ...
                    melee_buff += 2
                    break
            if GameConstants.ScientificNames.Hippopotamidae in x.binomial_nomenclature:
                if self.experience < GameConstants.Levels.level_3:
                    health_buff += 2
                else:
                    health_buff += 4
            if health_buff > 0 or ranged_buff > 0 or melee_buff > 0:
                x.shop_buff_stats(
