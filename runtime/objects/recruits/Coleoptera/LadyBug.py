...
class LadyBug(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=123,
                         sub_type_as_text="LadyBug", 
                         **kwargs)
    ...
    def passive_battle_ability(self,
        ...
        beetles = [x for x in info.friendly_recruits if
                   GameConstants.ScientificNames.Coleoptera in x.binomial_nomenclature]
        number_of_beetles = 0
        ...
        for i in range(number_of_beetles):
            x = battle_state.random.choice(beetles)
            if x is None:
                return
            x.add_status(Statuses.Carapace(counter=1), battle_state=battle_state,
                         state_set=state_set,
                         animation_event_sequence=animation_event_sequence,
                         group=g,
                         enemy=None,
    ...
    def shop_end_of_turn(self, shop_state: "ShopStateSerializer", state_set: "StateSet", stack_item: "StackItem",
        ...
        for x in shop_state.friendly_recruits:
            if GameConstants.ScientificNames.Coleoptera in x.binomial_nomenclature and GameConstants.Habitats.Grassland in x.binomial_nomenclature:
                x.shop_buff_stats(
