...
class MoleCricket(RecruitSerializer):
    def __init__(self, **kwargs):
        kwargs.pop("sub_type_as_int", None)
        kwargs.pop("sub_type_as_text", None)
        super().__init__(sub_type_as_int=118,
                         sub_type_as_text="MoleCricket", 
                         **kwargs)
    ...
    def passive_battle_ability(self,
        ...
        if self.experience < GameConstants.Levels.level_2:
            ...
            counters = 20
        self.add_status(status=Statuses.EntrenchedDugout(counter=counters), battle_state=battle_state,
                        state_set=state_set,
                        animation_event_sequence=animation_event_sequence,
                        enemy=None)
        logical_level = self.get_logical_level()
        logical_level = min(4, logical_level)
        self.battle_buff_stats(
            battle_state=battle_state,
            state_set=state_set,
            stack_item=stack_item,
            animation_event_sequence=animation_event_sequence,
            ranged=8 * logical_level,
            melee=8 * logical_level,
            armor=2 * logical_level,
        )
